...
class LocallyOrderedClusteringBuilder : public MortonCodeBasedBuilder<Bvh, Morton>
{
  ...
  std::pair<size_t, size_t> search_range(size_t i, size_t begin, size_t end) const
  {
    return std::make_pair(
      i > begin + search_radius ? i - search_radius : begin, std::min(i + search_radius + 1, end));
  }

  std::pair<size_t, size_t> cluster(const Node* bvh_restrict input, Node* bvh_restrict output,
    size_t* bvh_restrict neighbors, size_t* bvh_restrict merged_index, size_t begin, size_t end,
    size_t previous_end)
  {
    size_t next_begin = 0;
    size_t next_end = 0;
...
#pragma omp parallel if (end - begin > loop_parallel_threshold)
    {
      ...
      for (size_t i = chunk_begin; i < chunk_end; i++)
      {
        ...
        std::move_backward(distance_matrix.get(), distance_matrix.get() + search_radius,
        ...
        if (neighbors[j] == i)
        {
          if (i < j)
          {
            ...
            unmerged_node.bounding_box_proxy() =
              input[j].bounding_box_proxy().to_bounding_box().extend(input[i].bounding_box_proxy());
...
#pragma omp parallel for
    ...
    while (end - begin > 1)
    {
      auto [next_begin, next_end] = cluster(nodes.get(), nodes_copy.get(), auxiliary_data.get(),
