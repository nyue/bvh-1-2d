...
public:
  ...
  split(const BoundingBox<Scalar>& global_bbox, const Primitive* primitives, size_t primitive_count,
...
#pragma omp for
      for (size_t i = 0; i < primitive_count; ++i)
      {
        ...
        while (!stack.empty())
        {
          ...
          auto depth = std::min(Scalar(-1),
            std::floor(std::log2(bbox.largest_extent() / global_bbox.diagonal()[axis])));
          ...
          auto split_pos = global_bbox.min[axis] +
            std::round((mid_pos - global_bbox.min[axis]) / cell_size) * cell_size;
