...
public:
  ...
  void build(const BoundingBox<Scalar>& global_bbox, const BoundingBox<Scalar>* bboxes,
    const Vector3<Scalar>* centers, size_t primitive_count)
  {
    ...
    std::array<Scalar*, 3> costs = { cost_data.get(), cost_data.get() + primitive_count,
...
#pragma omp single
        {
          ...
          assert(axis < 2 || sorted_references[0] == bvh.primitive_indices.get() ||
...
#pragma omp for
        ...
        radix_sort.sort_in_parallel(sorted_keys, unsorted_keys, sorted_references[axis],
          unsorted_references, primitive_count, sizeof(Scalar) * CHAR_BIT);
      }
...
public:
  ...
  SweepSahBuildTask(Builder& builder, const BoundingBox<Scalar>* bboxes,
    const Vector3<Scalar>* centers, const std::array<size_t*, 3>& references,
    const std::array<Scalar*, 3>& costs, Mark* marks)
    : builder(builder)
    , bboxes(bboxes)
    , centers(centers)
