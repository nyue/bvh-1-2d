...
class LinearBvhBuilder : public MortonCodeBasedBuilder<Bvh, Morton>
{
  ...
  std::pair<size_t, size_t> merge(const Node* bvh_restrict input_nodes,
    Node* bvh_restrict output_nodes, const Level* bvh_restrict input_levels,
    Level* bvh_restrict output_levels, size_t* bvh_restrict merged_index,
    size_t* bvh_restrict needs_merge, size_t begin, size_t end, size_t previous_end)
  {
    size_t next_begin = 0;
    size_t next_end = 0;
...
#pragma omp for
      for (size_t i = begin; i < end - 1; ++i)
        needs_merge[i] = input_levels[i] >= input_levels[i + 1] &&
          (i == begin || input_levels[i] >= input_levels[i - 1]);
...
#pragma omp for nowait
      for (size_t i = begin; i < end; ++i)
      {
        if (needs_merge[i])
        {
          ...
          unmerged_node.bounding_box_proxy() =
            input_nodes[i].bounding_box_proxy().to_bounding_box().extend(
              input_nodes[i + 1].bounding_box_proxy());
...
#pragma omp for nowait
    ...
    while (end - begin > 1)
    {
      auto [next_begin, next_end] =
        merge(nodes.get(), nodes_copy.get(), input_levels, output_levels, auxiliary_data.get(),
          auxiliary_data.get() + node_count, begin, end, previous_end);
