...
int main()
{
  ...
  Ray ray(Vector3(0.25, 0.25, 0.0), // origin
    Vector3(0.0, -0.0, 1.0),        // direction
    0.0,                            // minimum distance
    100.0                           // maximum distance
  );

  return intersect_bvh_node<bvh::FastNodeIntersector<Bvh> >(node, ray) &&
      intersect_bvh_node<bvh::RobustNodeIntersector<Bvh> >(node, ray)
    ? 0
    : 1;
}
