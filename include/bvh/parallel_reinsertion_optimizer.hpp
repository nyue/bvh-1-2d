...
#pragma omp for
        for (size_t i = first_node; i < bvh.node_count; i += u)
        {
          ...
          auto lock = (uint64_t(as<uint32_t>(float(outs[i].second))) << 32) |
            (uint64_t(i) & UINT64_C(0xFFFFFFFF));
...
#pragma omp for
        for (size_t i = first_node; i < bvh.node_count; i += u)
        {
          ...
          bool is_conflict_free = std::all_of(conflicts.begin(), conflicts.end(),
            [&](size_t j) { return (locks[j] & UINT64_C(0xFFFFFFFF)) == i; });
          if (!is_conflict_free)
