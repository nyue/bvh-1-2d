...
struct Vector
{
  ...
  bvh_always_inline Vector safe_inverse() const
  {
    ...
    return Vector([&](size_t i) {
      return Scalar(1) /
        (std::fabs(values[i]) < threshold ? std::copysign(threshold, values[i]) : values[i]);
    });
  }

  bvh_always_inline Vector& operator+=(const Vector& other) { return *this = *this + other; }
...
template<typename Scalar, size_t N>
bvh_always_inline inline Vector<Scalar, N> operator+(
  const Vector<Scalar, N>& a, const Vector<Scalar, N>& b)
{
...
template<typename Scalar, size_t N>
bvh_always_inline inline Vector<Scalar, N> min(
  const Vector<Scalar, N>& a, const Vector<Scalar, N>& b)
{
...
template<typename Scalar, size_t N>
bvh_always_inline inline Vector<Scalar, N> max(
  const Vector<Scalar, N>& a, const Vector<Scalar, N>& b)
{
...
template<typename Scalar, size_t N>
bvh_always_inline inline Vector<Scalar, N> operator*(const Vector<Scalar, N>& a, Scalar s)
{
  return Vector<Scalar, N>([=](size_t i) { return a[i] * s; });
}

template<typename Scalar, size_t N>
bvh_always_inline inline Vector<Scalar, N> operator*(Scalar s, const Vector<Scalar, N>& b)
{
  return b * s;
}
