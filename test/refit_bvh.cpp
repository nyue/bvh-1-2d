...
{
  return std::all_of(bvh.nodes.get(), bvh.nodes.get() + bvh.node_count, [&](const Bvh::Node& node) {
    if (node.is_leaf())
    {
      return std::all_of(bvh.primitive_indices.get() + node.first_child_or_primitive,
    ...
    {
      auto left_bbox =
        bvh.nodes[node.first_child_or_primitive + 0].bounding_box_proxy().to_bounding_box();
      auto right_bbox =
        bvh.nodes[node.first_child_or_primitive + 1].bounding_box_proxy().to_bounding_box();
      return left_bbox.is_contained_in(node.bounding_box_proxy()) &&
