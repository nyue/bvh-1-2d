...
#pragma omp for
      ...
      traverse_in_parallel([&](size_t i) { primitive_counts[i] = bvh.nodes[i].primitive_count; },
        [&](size_t i) {
          ...
          if (left_primitive_count > 0 && right_primitive_count > 0)
          {
            ...
            auto collapse_cost = node.bounding_box_proxy().to_bounding_box().half_area() *
              (Scalar(total_primitive_count) - traversal_cost);
            auto base_cost =
              left_child.bounding_box_proxy().to_bounding_box().half_area() * left_primitive_count +
              right_child.bounding_box_proxy().to_bounding_box().half_area() *
      ...
      for (size_t i = 1; i < bvh.node_count; i++)
      {
        ...
        if (first_primitive != primitive_counts[i])
        {
          ...
          while (true)
          {
            ...
            {
              std::copy(bvh.primitive_indices.get() + node.first_child_or_primitive,
