...
struct BoundingBox
{
  ...
  BoundingBox() = default;
  bvh_always_inline BoundingBox(const Vector3<Scalar>& v)
    : min(v)
    , max(v)
  {
  }
  bvh_always_inline BoundingBox(const Vector3<Scalar>& min, const Vector3<Scalar>& max)
    : min(min)
    , max(max)
  {
  }
  ...
  bvh_always_inline Vector3<Scalar> diagonal() const { return max - min; }

  bvh_always_inline Vector3<Scalar> center() const { return (max + min) * Scalar(0.5); }
  ...
  bvh_always_inline Scalar largest_extent() const { return diagonal()[largest_axis()]; }

  bvh_always_inline bool is_contained_in(const BoundingBox& other) const
  {
    return max[0] <= other.max[0] && min[0] >= other.min[0] && max[1] <= other.max[1] &&
      min[1] >= other.min[1] && max[2] <= other.max[2] && min[2] >= other.min[2];
  }

  bvh_always_inline static BoundingBox full()
  {
    return BoundingBox(Vector3<Scalar>(-std::numeric_limits<Scalar>::max()),
