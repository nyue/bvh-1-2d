...
class SpatialSplitBvhBuildTask;
...
public:
  ...
  size_t build(const BoundingBox<Scalar>& global_bbox, const Primitive* primitives,
    const BoundingBox<Scalar>* bboxes, const Vector3<Scalar>* centers, size_t primitive_count,
    Scalar alpha = Scalar(1e-5), Scalar split_factor = Scalar(0.3))
  {
    ...
    std::array<Reference*, 3> references = { reference_data.get(),
      reference_data.get() + max_reference_count, reference_data.get() + 2 * max_reference_count };
...
#pragma omp single
      {
        BuildTask first_task(*this, primitives, accumulated_bboxes.get(), references,
          reference_count, primitive_count, spatial_threshold);
...
class SpatialSplitBvhBuildTask : public TopDownBuildTask
{
  ...
  struct ObjectSplit
  {
    ...
    ObjectSplit(Scalar cost = std::numeric_limits<Scalar>::max(), size_t index = 1, int axis = 0,
  ...
  struct SpatialSplit
  {
    ...
    SpatialSplit(
      Scalar cost = std::numeric_limits<Scalar>::max(), Scalar position = 0, int axis = 0)
      : cost(cost)
      , position(position)
      , axis(axis)
    {
...
#pragma omp taskloop if (end - begin > builder.task_spawn_threshold) grainsize(1) default(shared)
      ...
      {
        std::sort(references[axis] + begin, references[axis] + end,
          [&](const Reference& a, const Reference& b) { return a.center[axis] < b.center[axis]; });
      }
    }
    ...
    for (int axis = 0; axis < 3; ++axis)
    {
      ...
      {
        bbox.extend(references[axis][i].bbox);
        auto cost = bbox.half_area() * (i + 1 - begin) +
          accumulated_bboxes[i + 1].half_area() * (end - (i + 1));
  ...
  std::pair<WorkItem, WorkItem> allocate_children(Bvh& bvh, const WorkItem& item,
    size_t right_begin, size_t right_end, const BoundingBox<Scalar>& left_bbox,
    const BoundingBox<Scalar>& right_bbox, bool is_sorted)
  {
...
#pragma omp atomic capture
    ...
    auto left_split_ratio =
      left_cost + right_cost > 0 ? left_cost / (left_cost + right_cost) : Scalar(0.5);
    ...
    {
      std::move_backward(references[0] + right_begin, references[0] + right_end,
        references[0] + right_end + left_split_count);
      std::move_backward(references[1] + right_begin, references[1] + right_end,
        references[1] + right_end + left_split_count);
      std::move_backward(references[2] + right_begin, references[2] + right_end,
        references[2] + right_end + left_split_count);
    }
  ...
  std::pair<WorkItem, WorkItem> apply_object_split(
    Bvh& bvh, const ObjectSplit& split, const WorkItem& item)
  {
    ...
    auto partition_predicate = [&](const Reference& reference) {
      return reference_marks[reference.primitive_index];
    };
...
#pragma omp task if (item.work_size() > builder.task_spawn_threshold) default(shared)
      {
        std::stable_partition(references[other_axis[0]] + item.begin,
          references[other_axis[0]] + item.end, partition_predicate);
      }
#pragma omp task if (item.work_size() > builder.task_spawn_threshold) default(shared)
      {
        std::stable_partition(references[other_axis[1]] + item.begin,
          references[other_axis[1]] + item.end, partition_predicate);
      }
    }

    return allocate_children(
      bvh, item, split.index, item.end, split.left_bbox, split.right_bbox, true);
  }

  std::optional<std::pair<Scalar, Scalar> > run_binning_pass(
    SpatialSplit& split, int axis, size_t begin, size_t end, Scalar min, Scalar max)
  {
    ...
    for (size_t i = begin; i < end; ++i)
    {
      ...
      auto first_bin = std::min(
        bin_count - 1, size_t(std::max(Scalar(0), inv_size * (reference.bbox.min[axis] - min))));
      auto last_bin = std::min(
        bin_count - 1, size_t(std::max(Scalar(0), inv_size * (reference.bbox.max[axis] - min))));
      auto current_bbox = reference.bbox;
      for (size_t j = first_bin; j < last_bin; ++j)
      {
        auto [left_bbox, right_bbox] =
          primitives[reference.primitive_index].split(axis, min + (j + 1) * bin_size);
    ...
    for (size_t i = 0; i < bin_count - 1; ++i)
    {
      ...
      auto cost = left_count * current_bbox.half_area() +
        right_count * bins[i + 1].accumulated_bbox.half_area();
      if (cost < split.cost)
      {
        split.cost = cost;
        split.axis = axis;
    ...
    return found
      ? std::make_optional(std::make_pair(split.position - bin_size, split.position + bin_size))
      : std::nullopt;
  }

  SpatialSplit find_spatial_split(
    const BoundingBox<Scalar>& node_bbox, size_t begin, size_t end, size_t binning_pass_count)
  {
    SpatialSplit split;
    for (int axis = 0; axis < 3; ++axis)
    {
  ...
  std::pair<WorkItem, WorkItem> apply_spatial_split(
    Bvh& bvh, const SpatialSplit& split, const WorkItem& item)
  {
    ...
    while (left_end < right_begin)
    {
      ...
      if (item.split_end - right_end > 0)
      {
        ...
        references_to_split[right_end++] = Reference{ right_primitive_bbox,
          right_primitive_bbox.center(), reference.primitive_index };
        references_to_split[left_end++] =
          Reference{ left_primitive_bbox, left_primitive_bbox.center(), reference.primitive_index };
        left_count++;
        right_count++;
      }
    ...
    std::copy(references_to_split + item.begin, references_to_split + right_end,
...
public:
  ...
  SpatialSplitBvhBuildTask(Builder& builder, const Primitive* primitives,
    BoundingBox<Scalar>* accumulated_bboxes, const std::array<Reference*, 3>& references,
    size_t& reference_count, size_t primitive_count, Scalar spatial_threshold)
    : builder(builder)
...
#pragma omp atomic capture
      {
        first_primitive = reference_count;
    ...
    auto overlap = BoundingBox<Scalar>(best_object_split.left_bbox)
                     .shrink(best_object_split.right_bbox)
                     .half_area();
    if (overlap > spatial_threshold && item.split_end - item.end > 0)
    {
      auto binning_pass_count =
        static_cast<SpatialSplitBvhBuilder<Bvh, Primitive, BinCount>&>(builder).binning_pass_count;
      best_spatial_split =
        find_spatial_split(node.bounding_box_proxy(), item.begin, item.end, binning_pass_count);
    }
    ...
    auto max_split_cost =
      node.bounding_box_proxy().half_area() * (item.work_size() - builder.traversal_cost);
