...
void profile(const char* task, F f, size_t runs = 1)
{
  ...
  {
    std::cout << task << " took " << timings.front() << "/" << timings[timings.size() / 2] << "/"
...
static size_t compute_bvh_depth(const Bvh& bvh, size_t node_index = 0)
{
  ...
  {
    return 1 +
      std::max(compute_bvh_depth(bvh, node.first_child_or_primitive + 0),
...
static void usage()
{
  std::cout
    << "Usage: benchmark [options] file.obj\n"
       ...
       "  --permute               Activates the primitive permutation optimization (disabled by "
       "default).\n"
       ...
       "  --parallel-reinsertion  Activates the parallel reinsertion optimization (disabled by "
       "default).\n"
       "  --pre-split <percent>   Activates pre-splitting and sets the percentage of references "
       "(disabled by default).\n"
       "  --build-iterations <n>  Sets the number of construction iterations (equal to 1 by "
       "default).\n"
...
void render(const Camera& camera, const Bvh& bvh, const Triangle* triangles, Scalar* pixels,
  size_t width, size_t height, const Scalar* statistics_weights = NULL)
{
...
#pragma omp parallel for collapse(2) reduction(+ : traversal_steps, intersections)
  for (size_t i = 0; i < width; ++i)
  {
    for (size_t j = 0; j < height; ++j)
    {
      ...
      auto hit = CollectStatistics ? traverser.traverse(ray, intersector, statistics)
      ...
      else
      {
        if (CollectStatistics)
        {
          ...
          pixels[index] =
            std::min(statistics.traversal_steps * statistics_weights[0], Scalar(1.0f));
          pixels[index + 1] =
            std::min(statistics.intersections * statistics_weights[1], Scalar(1.0f));
...
int main(int argc, char** argv)
{
  ...
  Camera camera = { Vector3(0, 0, -10), Vector3(0, 0, 1), Vector3(0, 1, 0), 60 };
  bool permute = false;
  ...
  for (int i = 1; i < argc; ++i)
  {
    if (argv[i][0] == '-')
    {
      ...
      else if (!strcmp(argv[i], "--eye") || !strcmp(argv[i], "--dir") || !strcmp(argv[i], "--up"))
      {
        if (i + 3 >= argc)
        ...
        {
          case 'd':
            destination = &camera.dir;
            break;
          case 'u':
            destination = &camera.up;
            break;
          default:
            destination = &camera.eye;
            break;
        }
      ...
      else if (!strcmp(argv[i], "--width") || !strcmp(argv[i], "--height"))
      {
        if (i + 1 >= argc)
          return not_enough_arguments(argv[i]);
  ...
  std::function<size_t(
    Bvh&, const Triangle*, const BoundingBox&, const BoundingBox*, const Vector3*, size_t)>
    builder;
  if (!strcmp(builder_name, "binned_sah"))
  {
    builder = [](Bvh& bvh, const Triangle*, const BoundingBox& global_bbox,
                const BoundingBox* bboxes, const Vector3* centers, size_t primitive_count) {
  ...
  {
    builder = [](Bvh& bvh, const Triangle* triangles, const BoundingBox& global_bbox,
                const BoundingBox* bboxes, const Vector3* centers, size_t primitive_count) {
  ...
  profile("BVH construction",
    [&] {
      ...
      if (pre_split_factor > 0)
        std::tie(reference_count, bboxes, centers) =
          splitter.split(global_bbox, triangles.data(), triangles.size(), pre_split_factor);
      reference_count =
        builder(bvh, triangles.data(), global_bbox, bboxes.get(), centers.get(), reference_count);
      if (pre_split_factor > 0)
      ...
      if (permute)
        shuffled_triangles =
          bvh::permute_primitives(triangles.data(), bvh.primitive_indices.get(), reference_count);
    },
    build_iterations);
  ...
  std::cout << "BVH depth of " << compute_bvh_depth(bvh) << ", " << bvh.node_count << " node(s), "
  ...
  profile("Rendering", [&] {
    if (permute)
    {
      if (collect_statistics)
        render<true, true>(
          camera, bvh, shuffled_triangles.get(), pixels.get(), width, height, statistics_weights);
      else
    ...
    else
    {
      if (collect_statistics)
        render<false, true>(
          camera, bvh, triangles.data(), pixels.get(), width, height, statistics_weights);
      else
  ...
  for (size_t j = height; j > 0; --j)
  {
    for (size_t i = 0; i < width; ++i)
    {
      ...
      uint8_t pixel[3] = { static_cast<uint8_t>(
                             std::max(std::min(pixels[index] * 255, Scalar(255)), Scalar(0))),
