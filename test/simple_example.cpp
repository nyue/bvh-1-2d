...
static bool build_and_intersect_bvh(const std::vector<Primitive>& primitives)
{
  ...
  auto [bboxes, centers] =
    bvh::compute_bounding_boxes_and_centers(primitives.data(), primitives.size());
  ...
  if (auto hit = traverser.traverse(ray, primitive_intersector))
  {
    ...
    std::cout << "Hit primitive " << triangle_index << "\n"
              << "distance: " << intersection.t << "\n";
    if
      constexpr(std::is_same_v<Primitive, Triangle>)
      {
        std::cout << "u: " << intersection.u << "\n"
                  << "v: " << intersection.v << "\n";
      }
    return true;
  }
...
int main()
{
  ...
  triangles.emplace_back(Vector3(1.0, -1.0, 1.0), Vector3(1.0, 1.0, 1.0), Vector3(-1.0, 1.0, 1.0));
